...
// A CrawlTask represents a job for crawling related products from an origin product.
type CrawlTask struct {
	ID                      string    `db:"id"`                  // the entity's unique ID
	Completed               bool      `db:"completed"`           // whether or not the task was completed
	CreatedAt               time.Time `db:"created_at"`          // when the task was created
	OriginProductLocationID string    `db:"product_location_id"` // the ID of the product-location pair to be crawled from
}
